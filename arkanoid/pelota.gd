...
func _physics_process(delta: float) -> void:
	...
	if collision:
		...
		if collider.has_method("destruirse"):
			collider.destruirse()
			sumar_puntos()
	...
	if posision:
		if vidas != 0:
			...
			get_tree().change_scene_to_file("res://arkanoid/perdiste_juego.tscn")
...
func sumar_puntos(): 
	...
	if puntos==24 :
		get_tree().change_scene_to_file("res://arkanoid/ganaste_juego.tscn")
